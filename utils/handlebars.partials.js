...
module.exports = function(config) {
  ...
  var partialPaths = glob.sync(path.join(config.project.path, config.docs.partials.src));
  ...
  for(var i = 0; i < partialPaths.length; i++) {
    ...
    logger.info(config, 'Handlebars partial name converted from %s => %s', partialName, _partialName);
  }
